...
: Node("can_driver")
{
	...
	RCLCPP_INFO(this->get_logger(), "[UnifiedCanDriver] Unified CAN Driver ver. 0.0.3");
	...
	strncpy(ifr.ifr_name, GlobalSettings::can_bus_interface, sizeof(&GlobalSettings::can_bus_interface));
	...
	RCLCPP_INFO(this->get_logger(), "[UnifiedCanDriver] Starting ModuleLoader. . .");
	//dres_decoder = new Dres::Decoder::DresDecoder((rclcpp::Node*)this, &ifr);
	module_loader = new Module::ModuleLoader((rclcpp::Node*)this, &ifr);
	
	RCLCPP_INFO(this->get_logger(), "[UnifiedCanDriver] Starting CanService. . .");
	can_service = new CanSendService((rclcpp::Node*)this, &ifr);

	RCLCPP_INFO(this->get_logger(), "[UnifiedCanDriver] Starting CommandService. . .");
...
void UnifiedCanDriver::can_timer_callback()
{
	...
	{
		//dres_decoder->decode_dres(&dres_frame);
