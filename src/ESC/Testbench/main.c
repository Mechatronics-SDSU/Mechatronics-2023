...
#define VALS2TEST	4			// Number of values to test (each is tested for TES2RUN number of times)
...
int main(void){
	...
	uint8_t tes_values[VALS2TEST] = {map_2_(TEST_DUTY_), map_2_(TEST_DUTY_ * 2), map_2_(TEST_DUTY_ * 3), map_2_(TEST_DUTY_ * 4)};
	
	uint32_t results[TES2RUN * VALS2TEST] = {0x0000};
...
void testCycle(uint8_t *testVals, uint32_t *resultOut){
	...
	for(uint8_t n = 0; n < TES2RUN * VALS2TEST; n++){	// Run TES2RUN number of tests 4 times for each PWM freq (25%, 50%, 75%, 100%)
...
void printResults(uint8_t *testVals, uint32_t *results2Print){
	...
	for(uint8_t n = 0; n < TES2RUN * VALS2TEST; n++){
		if(!(n % TES2RUN)){
			if(n){
				t_ct_ += 1;
			}
