...
DVLModule::DVLModule(rclcpp::Node* ctx, Mailbox::MboxCan* mb)
	: DeviceModule(ctx, mb, 50, static_cast<uint16_t>(CanDriver::Device::WAYFDVL::ID), true, true, 16)
{
...
void DVLModule::timer_callback()
{
	if(do_device_polling && this->module_enable_ti)
	{
		...
		poll_frame.data[2] = static_cast<uint16_t>(CanDriver::Device::WAYFDVL::TOPIC_VEL_AND_DIST);
